...
/* turno/diretoria listas — 3 colunas: nome-fixo | barra-flex | valor */
.lista-perf { display: flex; flex-direction: column; gap: 0; flex: 1; overflow: hidden; }
.perf-item { display: grid; grid-template-columns: 5cqw 1fr auto; gap: 0.3cqw; align-items: center; padding: 0.28cqw 0; border-top: 1px solid var(--fio); font-size: 0.68cqw; }
.perf-item:first-child { border-top: 0; }
.perf-nome { color: var(--tinta-2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
function cartaoTendencia(ag) {
  // apenas meses com algum dado de interação, ordenados
  ...
  const maxVal = Math.max(META_MENSAL * 1.2, ...mesesInt.map(([,v])=>v));
  // SVG sem eixo Y — padding simétrico e limpo
  const W = 100, H = 58;
  const pad = { t: 8, r: 12, b: 13, l: 4 };
  const cw = W - pad.l - pad.r;
  const ch = H - pad.t - pad.b;
  const n  = mesesInt.length;
  const bw = Math.min(14, Math.max(3, (cw / n) * 0.6));

  const yx = v => H - pad.b - (v / maxVal) * ch;
  const cx = i => pad.l + (i + 0.5) * (cw / n);

  // grade horizontal sutil (sem labels Y)
  const gridLines = [0, META_MENSAL * 0.5, META_MENSAL].map(v =>
    '<line x1="' + pad.l + '" y1="' + yx(v).toFixed(1) + '" x2="' + (W - pad.r) + '" y2="' + yx(v).toFixed(1) + '" stroke="#E3E7E0" stroke-width="0.4"/>'
  ).join('');
  ...
  const barras = mesesInt.map(([m, v], i) => {
    ...
    const valLbl = v >= 1000 ? N0.format(v) : String(v);
    return '<rect x="' + x.toFixed(1) + '" y="' + y.toFixed(1) + '" width="' + bw.toFixed(1) + '" height="' + h.toFixed(1) + '"' +
           ' fill="' + fill + '" opacity="' + (isCur ? 1 : 0.5) + '" rx="0.8"/>' +
           '<text x="' + cx(i).toFixed(1) + '" y="' + (y - 1.8).toFixed(1) + '" text-anchor="middle" font-size="3.8" fill="' + fill + '" font-weight="700">' + valLbl + '</text>' +
           '<text x="' + cx(i).toFixed(1) + '" y="' + (H - pad.b + 4.5).toFixed(1) + '" text-anchor="middle" font-size="3.5" fill="#6E7873">' + mesFmt(m) + '</text>';
  }).join('');

  // meta — label colado no lado direito da linha
  const metaY = yx(META_MENSAL);
  const metaLine = '<line x1="' + pad.l + '" y1="' + metaY.toFixed(1) + '" x2="' + (W - pad.r) + '" y2="' + metaY.toFixed(1) + '"' +
                   ' stroke="#B54728" stroke-width="0.9" stroke-dasharray="2.5,1.5"/>' +
                   '<text x="' + (W - 0.5) + '" y="' + (metaY - 1.2).toFixed(1) + '" text-anchor="end" font-size="3.2" fill="#B54728" font-weight="700">1.000</text>';

  const svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg" style="overflow:visible;width:100%;display:block;flex:1">' +
              gridLines + barras + metaLine + '</svg>';

  return \`<article class="cartao">
    <div class="cartao-topo"><h3>Interações por mês</h3><span class="chip" data-s="nd">meta 1.000</span></div>
...
function cartaoTurno(ag) {
  ...
  const itens = ord.map(([t, v]) => {
    ...
    return \`<div class="perf-item">
      <span class="perf-nome">\${t}</span>
      <div class="rank-barra"><i style="width:\${pBar}%;background:var(--verde-claro)"></i></div>
...
function cartaoDir(ag) {
  ...
  const itens = ord.map(([d, v]) => {
    const p    = total ? v / total : 0;
    const pBar = (v / maxV * 100).toFixed(1);
    const nm   = d.length > 18 ? d.slice(0,16)+"…" : d;
    return \`<div class="perf-item">
      <span class="perf-nome" title="\${d}">\${nm}</span>
      <div class="rank-barra"><i style="width:\${pBar}%;background:var(--verde-claro)"></i></div>
      <span class="perf-val">\${N0.format(v)} <span class="perf-pct">(\${pct(p)})</span></span>
    </div>\`;
  }).join("");
